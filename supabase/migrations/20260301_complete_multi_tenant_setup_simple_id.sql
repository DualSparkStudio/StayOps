-- ============================================
-- PHASE 1: COMPLETE MULTI-TENANT SETUP (Simple Integer IDs)
-- ============================================
-- This migration converts the single-tenant system to multi-tenant
-- Uses simple INTEGER IDs instead of UUIDs
-- 
...
-- Reset the sequence to start from 2 for next property
SELECT setval('properties_id_seq', (SELECT MAX(id) FROM properties));

-- Link all existing data to Grand Valley (property_id = 1)
UPDATE rooms 
SET property_id = 1
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'resort_closures') THEN
        EXECUTE 'UPDATE resort_closures SET property_id = 1 WHERE property_id IS NULL';
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'whatsapp_sessions') THEN
        EXECUTE 'UPDATE whatsapp_sessions SET property_id = 1 WHERE property_id IS NULL';
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'features') THEN
        EXECUTE 'UPDATE features SET property_id = 1 WHERE property_id IS NULL';
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'house_rules') THEN
        EXECUTE 'UPDATE house_rules SET property_id = 1 WHERE property_id IS NULL';
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'faqs') THEN
        EXECUTE 'UPDATE faqs SET property_id = 1 WHERE property_id IS NULL';
    END IF;
END $$;
...
-- ============================================
-- 
-- ✅ Properties table created with INTEGER IDs
-- ✅ property_id added to all tables (INTEGER type)
-- ✅ Grand Valley created as property ID = 1
-- ✅ All existing data linked to Grand Valley
-- ✅ Row Level Security enabled
-- ✅ Indexes created for performance
-- 
-- Property IDs will be: 1, 2, 3, 4, etc. (simple integers)
-- 
-- Next steps:
